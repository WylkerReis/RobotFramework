...
E informo campos obrigatórios
   ...
   # Estrutura de seleção de página
   ...
   END
   # Estrutura de seleção do campo Tipo 
   IF      '${tipo}' == 'Fixo' 
   ...
   Sleep      5 
   # Estrutura de retry do campo Arvore Local
   ...
   Sleep      10
   # Estrutura de seleção do campo Família
   ...
   END
   Wait Until Element Is Not Visible   xpath: //*[@id="modal-loading"]  40
   # Estrutura de seleção do campo Classe 
   ...
   END
   # Estrutura de seleção de inserção de Microparada
   ...
   # Estrutura de decisão sobre Salvamento realizado ou persistência da tela de adição 
   ${STATUS}=    Run Keyword And Return Status      Page Should Contain Button         id:formMestreDetalhe:salvarButton    
   IF      '${STATUS}' == True
      Click Element   id:formResultado:botaoAdicionar    
   ELSE IF     '${STATUS}' 
